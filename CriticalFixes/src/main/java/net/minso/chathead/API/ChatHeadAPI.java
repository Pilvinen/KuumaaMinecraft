...
public class ChatHeadAPI {
    ...
    public BaseComponent[] getHead(UUID uuid, boolean overlay, SkinSource skinSource) {
        ...
        switch (skinSource) {
            ...
            case CRAFATAR:
                ...
                break;
            case LOCALFILE:
                hexColors = getPixelColorsFromLocalSkin();
                break;
        }
    ...
    private BaseComponent[] getHeadFromBase64(UUID uuid, String base64) {
        // Retrieve the hexadecimal colors array representing the player's skin based on the specified skin source
        String[] hexColors = null;
        hexColors = getPixelColorsFromBase64(base64);
        // Check if the retrieved colors array is valid (has at least 64 elements)
        if (hexColors == null || hexColors.length < 64) {
            throw new IllegalArgumentException("Hex colors must have at least 64 elements.");
        } //TODO add error handling
    ...
    public String getHeadAsStringFromBase64(UUID uuid, String base64) {
        return TextComponent.toLegacyText(
                this.getHeadFromBase64(uuid, base64)
        );
    }
    ...
    private String[] getPixelColorsFromLocalSkin() {
        String[] colors = new String[64]; // Initialize an array to store the pixel colors
        try {
            InputStream stream = getClass().getResourceAsStream("/images/faces/steve.png");

            assert stream != null;

            BufferedImage skinImage = ImageIO.read(stream);
    ...
    private String[] getPixelColorsFromBase64(String base64String) {
        String[] colors = new String[64];
        try {
            // Decode the Base64 string into a string
            String decodedString = new String(Base64.getDecoder().decode(base64String));
            // Split the decoded string into an array of color codes
            String[] decodedColors = decodedString.split("(?<=\\G.{7})");

            int faceWidth = 8, faceHeight = 8; // Define dimensions of the face (8x8)

            int index = 0;
            // Iterate through each pixel of the avatar image and extract its color
            for (int y = faceWidth - 1; y >= 0; y--) {
                for (int x = 0; x < faceHeight; x++) {
                    // Convert RGB value to hexadecimal string representation and store it in the array
                    colors[index++] = decodedColors[x * faceWidth + y];
                }
            }
